...
# from My_New_Swin_LSTM_for_HTNet_Model import SwinLSTM
import numpy as np
import datetime
...
#computate the F1(f1_score) and (average_recall)
def confusionMatrix(gt, pred, show=False):
    ...
    if num_samples == 0:
        ...
        print('this epoch average_recall is nan')
    else:
        average_recall = TP / num_samples
    return f1_score, average_recall

#将真实情感标签和预测情感标签转换为独热编码形式,将多分类标签转换为向量表示的方法,从而利用confusionMatrix计算UF1，UAR
def recognition_evaluation(final_gt, final_pred, show=False):
    label_dict = {'negative': 0, 'positive': 1, 'surprise': 2}
    # Display recognition result,存放所有类别的f1_score和av_recall,最后取平均得UF1和UAR
    f1_list = []
    ar_list = []
    try:
        #遍历每一种情感类别，先转化为二分类表情做计算。相当于将问题转化为“是否是该情感类别”的二分类问题。
...
#读取所有图片，并根据 MTCNN 人脸检测器获取整张脸的坐标,并存放字典(名字,数组坐标)中。MTCNN返回的坐标就是5个关键点位置的坐标。
...
def whole_face_block_coordinates():
    ...
    # for i in range(0, m):
    for i in range(0, m):
        #从索引i处的df DataFrame中提取'sub'和'filename_o'列的值，并使用str()函数将它们转换为字符串。
        image_name = str(df['sub'][i]) + '_' + str(
            df['filename_o'][i]) + ' .png'
        # print(image_name)
        img_path_apex = base_data_src + '/' + df['imagename'][i]
        # cv2.imread()函数用于读取图像文件，它返回的是一个三维数组
        ...
        #处理了MTCNN无法检测图像中的面部标志的情况。如果没有找到地标，它会分配一组默认的地标坐标，以确保代码可以继续处理。
        #好像sub开头的照片都无法使用MTCNN获取面部标志，s开头的可以。两个数据集的差异？？？？？
        if batch_landmarks is None:
            # print( df['imagename'][i])
            #坐标的具体值代表了5个关键面部特征的大致位置
        ...
        #迭代每个面部地标的每个x和y坐标。裁剪，确保地标坐标在在有效范围内(可能对应于图像边界)。超出此范围的值可能不准确或无意义。
...
#根据整5个关键点位置坐标，从光流图像中裁剪出五个感兴趣区域 (左眼、左嘴唇、鼻子、右眼、右嘴唇)。
...
def crop_optical_flow_block():
    ...
    # Get train dataset，获取整张人脸的光流图
    # whole_optical_flow_path = './datasets/STSNet_whole_norm_u_v_os'
    whole_optical_flow_path = '/home/ff/dataset/lyl/casme2/casme2_STSNet_Whole_OF'
    ...
    for n_img in whole_optical_flow_imgs:
        ...
        four_part_coordinates = face_block_coordinates_dict[n_img]
         # four_part_coordinates[0][0] 和 four_part_coordinates[0][1]：分别表示左眼关键点的横坐标和纵坐标。
        # -7:7：表示在关键点坐标的基础上，向上下左右各扩展7个像素，形成一个15x15的区域。
...
class Fusionmodel(nn.Module):
  def __init__(self):
    ...
    # 全连接层：将 15 维的输入向量映射到 3 维的输出向量。
    self.fc1 = nn.Linear(15, 3) # 15->3
    # BatchNormalization;通过对每个批次的数据进行归一化，BatchNorm能保持数据分布一致，有助于避免梯度爆炸或消失。加速收敛
...
# def plot_loss(train_losses, val_losses, epoch, plot_every):
def plot_loss(train_losses, val_losses, epoch,Subject,save_dir):
    ...
    filename = f"{Subject}_{current_date}.png"
    ...
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.title(f'Training and Validation Loss at Epoch {epoch}')
    plt.legend()
    # plt.show()
    # 保存图形为PNG文件
...
def main(config):
    ...
    #初始化用于存储 ground truth (真实标签) 和预测结果的列表。gt=ground truth真实标签
    total_gt = []
    total_pred = []
    best_total_pred = []
    ...
    main_path = '/home/ff/dataset/lyl/casme2/casme2_STSNet_OF'
    subName = os.listdir(main_path)
    all_five_parts_optical_flow = crop_optical_flow_block()
    ...
    current_date = datetime.datetime.now().strftime("%Y-%m-%d")
    # 创建一个基于当前日期的文件名
    file_path = f"logfile_{current_date}_casme2_STSNet_OF.txt"
    ...
    for n_subName in subName:
        ...
        model = HTNet(
            image_size=28,
            patch_size=7,
            dim=256,  # 256,--96, 56-, 192    模型中的特征维度。
            heads=3,  # 3 ---- , 6-         在 Transformer 中使用的注意力头的数量。
            num_hierarchies=3,  # 3----number of hierarchies    模型中的层级数量。意味着模型在不同的尺度上处理图像。
            block_repeats=(2, 2, 8),#(2, 2, 8),------  每个层级中 Transformer 块的重复次数。这里设置为 (2, 2, 10)，表示第一层有 2 个块，第二层有 2 个块，第三层有 10 个块。
            # the number of transformer blocks at each heirarch  , starting from the bottom(2,2,20) -
            num_classes=3)

        # model = SwinLSTM( 
        #     img_size=(28, 28),
        #     patch_size=(7,7),
        #     embed_dim=96,  # 256,--96, 56-, 192    模型中的特征维度。
        #     num_heads=[3, 6, 12] ,  # 3 ---- , 6-         在 Transformer 中使用的注意力头的数量。
        #     depths=[12],  # 3----number of hierarchies    模型中的层级数量。意味着模型在不同的尺度上处理图像。
        #     # block_repeats=(2, 2, 10),#(2, 2, 8),------  每个层级中 Transformer 块的重复次数。这里设置为 (2, 2, 10)，表示第一层有 2 个块，第二层有 2 个块，第三层有 10 个块。
        #     # the number of transformer blocks at each heirarch  , starting from the bottom(2,2,20) -
        #     # num_classes=3,
        #     in_chans=3,
        #     window_size=7,
        ...
        if(config.train):
            ...
            print('load exists weight_path')
            print(f"cur weight {weight_path}")
        ...
        for epoch in range(1, epochs + 1):
            if (config.train):
                ...
                for batch in train_dl:
                    ...
                    x = batch[0].to(device)
                    y = batch[1].to(device)
                    # 新的backbone还没有改好，暂时多一个东西
                    # num_layers=[12]
                    # states = [None] * len(num_layers)
                    # yhat, state = model(x, states)
                    yhat= model(x)
                    loss = loss_fn(yhat, y)
                    loss.backward()
                    optimizer.step()
        ...
        with open(file_path,'a') as  file:
            ...
            file.write(f'Best Predicted  :{best_each_subject_pred} \n')
            file.write(f'best_UF1:  {round(best_UF1, 4)} | best_UAR: {round(best_UAR, 4)} \n')
            file.write(f'#################################################### \n')
        #print('#################################################### ')

        #plot_loss(train_loss_graph, val_loss_graph, epoch, n_subName, save_graph) 
...
if __name__ == '__main__':
    ...
    # input parameters   true默认训练模型，训练完之后改成false。
